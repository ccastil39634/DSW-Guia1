...
public class Parqueadero {
    ...
    public String metodo1() {
        String placaVeh = null;
        int horaMenor = 21;

        for (Puesto puesto: puestos) {
            if (puesto.estaOcupado()) {
                Carro carro = puesto.darCarro();
                if (carro.darHoraLlegada() < horaMenor) {
                    horaMenor = carro.darHoraLlegada();
                    placaVeh = carro.darPlaca();
                }
            }
        }

        return ("El vehiculo que lleva más tiempo es el " + placaVeh);
    }
    ...
    public String metodo2() {
        ...
        if (puestosLibres == 0) {
            Disponibilidad = "El parqueadero está lleno";
        }
        else {
            Disponibilidad = "Hay puestos disponibles";
        }
