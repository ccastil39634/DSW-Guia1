...
import static universidadean.feriaempresarial.mundo.Puesto.*;
...
public class FeriaEmpresarial {
    ...
    public void ingresarEmpresaExpositora(String nNombreEmpresa, int nNumeroPersonasExpositoras, String zonaPuesto, int numeroPuesto) throws Exception {
        ...
        if (zonaPuesto.equals(ZONA_NORTE)) {
        ...
        else if (zonaPuesto.equals(ZONA_ORIENTE)) {
        ...
        else if (zonaPuesto.equals(ZONA_SUR)) {
    ...
    public String metodo1() {
        ...
        int ExpOccidente = 0;
        int ExpCentro = 0;
        int numMaxExpositores = 0;
        this.totalExpositores = 0;
        String ZonaMaxExpositores = null;
        Puesto p = null;

        for (Puesto puesto : puestos) {
            if (puesto.darZona().equals(ZONA_SUR) && puesto.estaOcupado()) {
                p = puesto;
                String nombreEmpresa = p.darNombreEmpresa();
                for (Empresa empresa : empresas) {
                    if (empresa.darNombre().equals(nombreEmpresa)) {
                        ExpSur += empresa.darNumeroPersonasAsistentes();
                        this.totalExpositores += empresa.darNumeroPersonasAsistentes();
                    }
                }
            } else if (puesto.darZona().equals(ZONA_NORTE) && puesto.estaOcupado()) {
                p = puesto;
                String nombreEmpresa = p.darNombreEmpresa();
                for (Empresa empresa : empresas) {
                    if (empresa.darNombre().equals(nombreEmpresa)) {
                        ExpNorte += empresa.darNumeroPersonasAsistentes();
                        this.totalExpositores += empresa.darNumeroPersonasAsistentes();
                    }
                }
            } else if (puesto.darZona().equals(ZONA_ORIENTE) && puesto.estaOcupado()) {
                p = puesto;
                String nombreEmpresa = p.darNombreEmpresa();
                for (Empresa empresa : empresas) {
                    if (empresa.darNombre().equals(nombreEmpresa)) {
                        ExpOriente += empresa.darNumeroPersonasAsistentes();
                        this.totalExpositores += empresa.darNumeroPersonasAsistentes();
                    }
                }
            } else if (puesto.darZona().equals(ZONA_OCCIDENTE) && puesto.estaOcupado()) {
                p = puesto;
                String nombreEmpresa = p.darNombreEmpresa();
                for (Empresa empresa : empresas) {
                    if (empresa.darNombre().equals(nombreEmpresa)) {
                        ExpOccidente += empresa.darNumeroPersonasAsistentes();
                        this.totalExpositores += empresa.darNumeroPersonasAsistentes();
                    }
                }
            } else if (puesto.darZona().equals(ZONA_CENTRO) && puesto.estaOcupado()) {
                p = puesto;
                String nombreEmpresa = p.darNombreEmpresa();
                for (Empresa empresa : empresas) {
                    if (empresa.darNombre().equals(nombreEmpresa)) {
                        ExpCentro += empresa.darNumeroPersonasAsistentes();
                        this.totalExpositores += empresa.darNumeroPersonasAsistentes();
                    }
                }
            }
        ...
        if (ExpNorte > numMaxExpositores) {
            numMaxExpositores = ExpNorte;
            ZonaMaxExpositores = "Zona Norte";
        }
        if (ExpSur > numMaxExpositores) {
            numMaxExpositores = ExpSur;
            ZonaMaxExpositores = "Zona Sur";
        }
        if (ExpOriente > numMaxExpositores) {
            numMaxExpositores = ExpOriente;
            ZonaMaxExpositores = "Zona Oriente";
        }
        if (ExpOccidente > numMaxExpositores) {
            numMaxExpositores = ExpOccidente;
            ZonaMaxExpositores = "Zona Occidente";
        }
        if (ExpCentro > numMaxExpositores) {
            numMaxExpositores = ExpCentro;
            ZonaMaxExpositores = "Zona Centro";
        }

        return "Zona con el máximo de expositores: \n" + ZonaMaxExpositores;
    }
    ...
    public String metodo2() {
        metodo1();
        int puntoEquilibrio = 60;
        int maxExpositores = 237;
        int totalExp = this.totalExpositores;
        double porcentajeExp = 0;
        String equilibrio = null;

        porcentajeExp = (double) totalExp/maxExpositores;
        porcentajeExp = porcentajeExp * 100;

        if (porcentajeExp < puntoEquilibrio) {
            equilibrio = "El punto de equilibrio no se ha cumplido";
        }
        else {
            equilibrio = "El punto de equilibrio se ha cumplido";
        }
